...
{
    Clear-Host
    Write-Host $(If ($selectedLines -eq 1) {"[x]"} Else {"[ ]"}) "Whole text is in green" -ForegroundColor $(If ($currentLine -eq 0) {"Green"} Else {"White"})
    ...
    if($pressedKey.VirtualKeyCode -eq 40) {
        ...
        $selectedLines = $selectedLines + 1 + $currentLine
    }
}